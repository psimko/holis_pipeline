...
def combine_masks(detection_masks_folder, vol_unmixed_folder, output_folder_scale, output_combined_mask):
    ...
    if os.path.exists(mask_location):
        ...
        print(f'lazy_tiff_stack {dask_zarray.shape}')
        ...
        for mask in masks:
            ...
            combined_mask[chunk_slices[0], chunk_slices[1], chunk_slices[2]] |= mask_raw_space   # union not overwrite """
        ...
        tifffile.imwrite(mask_location, (combined_mask > 0).astype(np.uint8) * 255, metadata={'axes': 'ZYX'})
        return mask_location 
