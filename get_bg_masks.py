...
base_nuclei = os.path.basename(NUCLEI_FLI)                                  # e.g. NPBB328-...-272.fli.zst
base_colors = os.path.basename(COLORS_FLI)                             
...
# Define path for bg masks
bg_nuclei_fileName = f'bg_nuclei_mask_z{z_str}.npy'       # Don't change the name here - or if you do make sure to change it in preprocess_v2_sep2025.py
bg_colors_fileName = f'bg_colors_mask_z{z_str}.npy'
NUCLEI_BG_MASK_PATH = os.path.join(OUTPUT_DIR, bg_nuclei_fileName)
COLORS_BG_MASK_PATH = os.path.join(OUTPUT_DIR, bg_colors_fileName)
...
def main():
    ...
    log.info("Generating Bg masks")

    if os.path.exists(NUCLEI_BG_MASK_PATH):
        log.info(f"Nuclei bg mask already present at {NUCLEI_BG_MASK_PATH}, skipping read.")
        nuclei_bg_mask_path = NUCLEI_BG_MASK_PATH
    else:
        nuclei_bg_mask_path = get_mask(nuclei_dir, NUCLEI_BG_MASK_PATH)
        log.info(f"Saved nuclei bg mask to {nuclei_bg_mask_path}")

    if os.path.exists(COLORS_BG_MASK_PATH):
        log.info(f"Colors bg mask already present at {COLORS_BG_MASK_PATH}, skipping read.")
        colors_bg_mask_path = COLORS_BG_MASK_PATH
    else:
        colors_bg_mask_path = get_mask(colors_dir, COLORS_BG_MASK_PATH)
        log.info(f"Saved colors bg mask to {colors_bg_mask_path}")
